...
#for i in range(590000,599999): # to grab by range comment some lines below
with open('yndr_id.lst') as fp:
  for cnt, line in enumerate(fp):
    try:
      i = int(line)
      page = requests.get("https://yande.re/post/show/"+str(i))
      if page:
        ...
        try:
          if not str(rresp)[51:-12].startswith("Box"):
            print(str(i)+";"+str(normalize('NFKD',str(rresp)[51:-12]).encode('ascii', 'ignore')),flush=True)
        except:
