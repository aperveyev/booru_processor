...
myconnection = cx_Oracle.connect('user','pwd','db')
mycursor = myconnection.cursor()
mycursor.execute(" select id, fname, s_url, rating from yndr_rip where done=0 order by id fetch first 20000 rows only ")
insconnection = cx_Oracle.connect('user','pwd','db')
inssql = ("update yndr_rip set done=1 where id = :v_id ")
errsql = ("update yndr_rip set done=-1 where id = :v_id ")

for (cur) in mycursor:
  try:
    headerrs = {'User-agent': 'Mozilla/5.0 (Windows NT 6.3; rv:36.0) Gecko/20100101 Firefox/36.0'}
    p = requests.get(cur[2], headers=headerrs)
    with open('SAMPLE'+cur[3]+'\\' + cur[1], 'wb') as outfile:
